...
Concretely, ```name```, ```category``` and ```designer``` were converted from character to categorical variables. In the ```designer``` column, blank strings, strings containing digits and strings prefixed by “IKEA of Sweden” were converted to missing values (```NA```). Furthermore, both the price and old price were converted to double values and the currency was changed from Saudi Arabian Riyals to Euros based on the exchange rate from the time the data set was obtained by the author (See section \@ref(theoretical-background)).

The data set had a peculiarity where some rows were exact duplicates except for values in the ```category``` vector. The authors considered multiple approaches to handle these data duplications without losing information about the category of an item. 
...
Overall this would lead to having more small categories which increases the difficulty in applying a regression model due to overfitting (For more information on overfitting, see section \@ref(overfitting)).
...
To better facilitate the comparison of the different sizes of furniture items, the size in cubic meters was computed based on the depth, width and height values, and added as a column vector for further analysis. Options with only two of the measures filled were set to `NA`.
...
The following sections explore the given data set based on the eight step data exploration protocol proposed by @Zuur2010. 
...
```{r, outliers, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap="Boxplots showing the Price in € based on Independent Variables", out.width="100%"}
...
Within each name, and within each category the variance is heterogenous (see fig. \@ref(fig:homogeneity)). However, looking at both name and category in conjunction, it is possible to explore homogeneous patterns of variance for price.

Due to the limited scope and length of this paper, the authors were not able to explore all variable combinations for the five categorical variables ($2^5=32$).


```{r, homogeneity, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap="A Plot showing the Homogeneity of category for selected combinations of name and category", out.width="100%"}
tidy_ikea %>%
...
All numerical variables (```price```, ```old_price``` and ```size_m3```) are not arranged along a normal distribution (see fig. \@ref(fig:normality)), but rather follow an exponential decay function ($e^{-x}$).


```{r, normality, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap="A Histogram Showing Numerical Variables with a Bin Width of 100 Euro", out.width="100%"}
tidy_ikea %>%
...
The abscence of many of the old price values is due to the fact that most items were not on sale and thus do not have an old price.

```{r, missing-values, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap="A Plot Showing the Percentage of Missing Values", out.width="100%"}
...
```old_price_eur``` has a linear relationship (see fig. \@ref(fig:relationship-old-price)) whereas ```size_m3``` fits a second order polynomial to price (see fig. \@ref(fig:relationship-size-m3)).


```{r relationship-old-price, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap="A Plot Showing the Relationshihp of Price and Old Price", out.width="100%"}
tidy_ikea %>%
...
```{r relationship-size-m3, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.cap="A plot Showing the Relationship of Price size_m3", out.width="100%"}
tidy_ikea %>%
...
In the given data set there are three numerical and three categorical variables which can be explored in this form of interaction. For the numerical variables, ```old_price_eur``` has a very strong relationship with ```price``` (see fig \@ref(fig:relationship-old-price)). A more detailed breakdown by the categorical variables would not reveal new information. This leaves the exploration of ```size_m3``` and ```price_eur``` subdivided by designer, name and category resulting in (${3 \choose 2} = 3$) combinations of coplots.

#### Interaction of size and price coplotted by designer and name {#size-price-interaction}
\textcolor{gray}{by P. Krück}

It is unlikely that there is an interaction between size and price split by name and designer as is indicated by the non-parallelism of the fitted lines in the coplot @\ref(fig:coplot-code).
...
Unfortunately, the authors of this papers were not able to fully explore all combinations.
...
According to the error message, the linear model predicted infinite values for some of the coplotted combinations for both amalgamations that would not render correctly. 
...
Still there seemed to be infinite values computed by the linear method. The authors hypothesized that those values were caused by a division by 0 of the internal algorithm mechanics. This however proved to be wrong after applying the respective filters.
